...
if __name__ == '__main__':
    ...
    keyboard = types.InlineKeyboardMarkup(row_width=2)
    keyboard.add(
        types.InlineKeyboardButton("🤖 Alloy",   callback_data="voice_alloy"),
        types.InlineKeyboardButton("🔊 Echo",    callback_data="voice_echo"),
        types.InlineKeyboardButton("📖 Fable",   callback_data="voice_fable"),
        types.InlineKeyboardButton("🎙️ Onyx",    callback_data="voice_onyx"),
        types.InlineKeyboardButton("✨ Nova",    callback_data="voice_nova"),
        types.InlineKeyboardButton("🌟 Shimmer", callback_data="voice_shimmer"),
    )
    ...
    await TTSStates.waiting_for_voice.set()

# ================================================================
# توليد الصوت
# ================================================================
@dp.callback_query_handler(lambda c: c.data.startswith('voice_'), state=TTSStates.waiting_for_voice)
async def handle_voice(call: types.CallbackQuery, state: FSMContext):
    chat_id   = call.message.chat.id
    ...
    if not user_text:
        await call.answer("❌ انتهت الجلسة، أرسل النص من جديد.", show_alert=True)
        await state.finish()
        return

    await call.message.edit_text("🎙️ <b>جاري تحويل النص لصوت... ⏳</b>")

    try:
        encoded = quote(user_text)
        url = f"https://text.pollinations.ai/{encoded}?model=openai-audio&voice={voice_key}"
        resp = requests.get(url, timeout=30, verify=False)

        if resp.status_code == 200 and len(resp.content) > 500:
            file_path = f"/sdcard/tts_{chat_id}.mp3"
            with open(file_path, "wb") as f:
                f.write(resp.content)

            voice_name = VOICES.get(voice_key, voice_key)
            with open(file_path, "rb") as audio:
                await bot.send_voice(
                    ...
                    caption=(
                        f"✨ <b>تم التحويل!</b>\n"
                        f"✏️ <b>النص:</b> « {user_text[:60]} »\n"
                        f"🎙️ <b>الصوت:</b> {voice_name}"
                    )
                )
            try:
                os.remove(file_path)
            ...
            logging.error(f"TTS error: {resp.status_code}")
            await bot.send_message(chat_id, "❌ فشل التحويل، حاول مرة أخرى.")
